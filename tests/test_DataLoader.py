...
class TestPlanktonDataLoader(unittest.TestCase):
    ...
    def test_dataloader_klas_data(self):
        new_config = dict(use_klas_data=True, use_planktonnet_data=False, preload_dataset=False)
    ...
    def test_dataloader_on_planktonnet(self):
        new_config = dict(use_klas_data=False, use_planktonnet_data=True, preload_dataset=False)
        CONFIG.update(new_config)
        dl = PlanktonDataLoader()
        dl.setup()
